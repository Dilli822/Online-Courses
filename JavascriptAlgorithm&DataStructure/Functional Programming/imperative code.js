...
// finalTabs.tabs should be ['FB', 'Gitter', 'Reddit', 'Twitter', 'Medium', 'new tab', 'Netflix', 'YouTube', 'Vine', 'GMail', 'Work mail', 'Docs', 'freeCodeCamp', 'new tab']
  ...
  Window.prototype.tabOpen = function(tab) {
    this.tabs.push(tab || 'new tab'); // Use the provided tab parameter if available, otherwise use 'new tab'
    console.log("open a tab", this.tabs);
    return this;
  };
  
  Window.prototype.tabClose = function(index) {
    const tabsBeforeIndex = this.tabs.slice(0, index); // Use slice instead of splice to avoid modifying the original array
